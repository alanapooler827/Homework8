...
4 categorical variables, 10 numeric variables

We will convert 3 of the character variables (holiday, functioning day, and seasons) to factors. The last character variable, Date, needs to be converted as well.

```{r}
sapply(df, class)
```

Convert date from a character variable to a date for use in later steps.

```{r}
df$Date <- dmy(df$Date)
```

Convert remaining 3 categorical variables to factors, rename columns to be consistent & easy to use
...
-   levels of 'Seasons': Autumn, Spring, Summer, Winter

-   levels of 'Holiday': Holiday, No Holiday

-   levels of 'Functioning Day': Yes, No
...
-   Calculate daily averages of all 8 weather related variables.
...
Measures of spread center for total daily rented bike count
...
-   We see the same pattern of more bike rentals when there is no holiday
...
-   We see the same pattern of the most bike rentals in the summer and the least by far in the winter
...
Set up linear model fit to use when fitting models
...
Create workflow and fit model using recipe 1
...
Create workflow and fit model using recipe 2
...
Create workflow and fit model using recipe 3
